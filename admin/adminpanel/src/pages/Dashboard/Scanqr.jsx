          ...
          return new Promise((resolve, reject) => {
            ...
            const checkElement = () => {
              ...
              if (Date.now() - startTime > timeout) {
                reject(
                  new Error(
                    `Element with id="${selector}" not found within ${timeout}ms`
                  )
                );
                return;
              }
            ...
            setTimeout(async () => {
              ...
              try {
                ...
                if (res.data.ScanStatus === "Ticket already scanned") {
                  ...
                  setScanResult(
                    `⚠️ Ticket already scanned: ${res.data.name} (${res.data.status}) (${res.data.ScanStatus})`
                  );
                } else {
                  new Audio("/correct.mp3").play().catch(() => {});
                  setScanResult(
                    `✅ Ticket Valid: ${res.data.name} (${res.data.status}) (${res.data.ScanStatus})`
                  );
                }
              } catch (err) {
                console.log(err);
                new Audio("/errorsound.mp3").play().catch(() => {});
                setScanResult(
                  "❌ Invalid or already used ticket! or Failed to connect to database"
                );
              }
            }, 1000);
          },
          ...
          <div className="flex flex-col items-center justify-center mb-8">
            <div className="relative">
              ...
              <div className="relative w-80 h-80 sm:w-96 sm:h-96 bg-black rounded-3xl shadow-2xl border-4 border-green-300 overflow-hidden">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div
                    className={`relative w-64 h-64 border-4 transition-all duration-300 ${
                      capturing
                        ? "border-green-400 shadow-green-glow"
                        : "border-white/50"
                    }`}
                  >
                    {/* Corner decorations */}
                    ...
                    {capturing && (
                      <>
                        ...
                        {/* Checkmark animation */}
                        <div className="absolute inset-0 flex items-center justify-center">
                          <div className="text-6xl text-green-400 animate-bounce">
                            ✓
                          </div>
                        </div>
                      </>
